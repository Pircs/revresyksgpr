...
//@@@int sec = (rand()%5)*100 + 0;printf("*[%d] --> ", sec/100); Sleep(sec); printf("|||\n");
		...
		{	
			...
			{	
				...
				m_Fields.clear();
				for (int i=0; i<m_uiFieldsCount; ++i)
				{
					MYSQL_FIELD* pFields = mysql_fetch_field(m_Recordset);
					m_Fields[i] = *pFields;
					//MYSQL_FIELD * pField = mysql_fetch_field(pRes);
					//if(!pField)
					//{
...
CMyField CMyRecordset::Fields(const std::string& strField)
{
	int nIndex = -1 ;
	for (UINT i=0; i < m_uiFieldsCount; i++)
	{	
		if (strField==m_Fields[i].name)
		{	
			nIndex = i;
			break;
		}
	...
	{
		::LogSave("Error: field %s not found in CMyRecordset::Fields", strField.c_str());
