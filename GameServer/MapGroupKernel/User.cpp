...
#define  XPFULLSOUNDEFFECT		1000000
...
DWORD CUser::GetMaxLife()
{
	...
	{
		CItem** ppEquip = GetEquipItemPtr(i);
		CHECKF(ppEquip);
		CItem*& pEquip = *ppEquip;
		if(pEquip)
...
//						&& !pMap->IsDeadIsland())	// 死亡岛杀人不闪蓝，不加PK值
				...
				&& pTargetUser->GetPk() < PKVALUE_BLACKNAME)
			{
				...
				if (!QueryStatus(STATUS_CRIME))
				{
#define  CRIME_TIME_PERMURDER	30	//2*60;		// 闪蓝时间改为30秒
		...
		if (bDropItem)
		{
			...
			if (setPosition.size() > 0)
			{
				for (int i=0; i<__min(nEquipDropNum, setPosition.size()); i++)
				{
					...
					CItem* pEquip = GetEquipItemByPos(idPosition);
...
void CUser::BroadcastRoomMsg(Msg* pMsg, bool bSendSelf /*= false*/)		
{ 
...
void CUser::BroadcastMapMsg(Msg* pMsg, bool bSendSelf /*= false*/)		
{ 
	//CGameMap* pMap = MapManager()->GetGameMap(GetMapID());
	CGameMap* pMap = this->GetMap();
	if(pMap)
...
void CUser::BroadcastFriendsMsg	(Msg* pMsg)
{
	if (!pMsg)
		return;
	...
	for(int i = 0; i < nAmount; i++)
	{
		...
		if(pFriend)
		{
			...
			{
				MapGroup(PID)->QueryIntraMsg()->TransmitWorldMsgByUserID(pMsg, pFriend->GetFriendID());
			}
		}
	}
...
bool CUser::AppendItem(ItemInfoStruct* pInfo)
{
	...
	{
		CItem*& refpEquip = this->GetEquipItemRef(pInfo->nPosition);
		IF_OK(!refpEquip)
		{
			refpEquip = pItem;
			pItem = NULL;		// 不放入背包
...
void CUser::SendSystemBBS(LPCTSTR szText)
{
	...
	IF_OK(msg.Create(this->GetName(), ALLUSERS_NAME, szText, NULL, 0xffffff, _TXTATR_MSG_SYSTEM))
		MapGroup(PID)->QueryIntraMsg()->TransmitWorldMsgByUserID(&msg, idUser);
}
...
bool CUser::EnhanceEudemon(OBJID idEudemonItem, OBJID idGem)
{
	...
	nRate /= pow(2.0f, nAdditionLevel);	// 每+1成功率降低一半
...
bool CUser::AttachEudemon(OBJID idItem)
{
	...
	// 通知客户端
	MsgItem msg;
	if (msg.Create(idItem, ITEMACT_ATTACH_EUDEMON, ITEMPOSITION_NONE))
		SendMsg(&msg);
...
void CUser::DetachEudemon(CItem* pItem)
{
	...
	// 通知客户端
	MsgItem msg;
	if (msg.Create(m_idLinkEudemonItem, ITEMACT_DETACH_EUDEMON, ITEMPOSITION_NONE))
		SendMsg(&msg);
...
void CUser::NotifyTutorStatus(int nStatus)
{
	...
	if (m_pTutor)
	{
		...
		MapGroup(PID)->QueryIntraMsg()->TransmitWorldMsgByUserID(&msg, m_pTutor->GetTutorID());
	}
	...
	for (; it!=m_setStudent.end(); it++)
	{
		...
		if (pTutor)
		{
			...
			if (pUser)
				...
				MapGroup(PID)->QueryIntraMsg()->TransmitWorldMsgByUserID(&msg, pTutor->GetUserID());
			...
			if (msgNotify.Create(SYSTEM_NAME, pTutor->GetUserName(), szMsg, NULL, 0xff0000, _TXTATR_SYSTEM))
			{
				if (pUser)
					...
					MapGroup(PID)->QueryIntraMsg()->TransmitWorldMsgByUserID(&msgNotify, pTutor->GetUserID());
			}
		}
	}
}
...
{ /*
	...
	for(int i=ITEMPOSITION_EQUIPBEGIN; i < ITEMPOSITION_EQUIPEND; i++)
	{
		CItem* pEquipItem = GetEquipItemByPos(i);
