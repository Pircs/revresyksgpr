...
{
	CMonster* pNpc = CMonster::CreateNew();
	if(pNpc)
	{
		...
		if(pType)
		{
			...
			CGameMap* pMap = MapManager()->GetGameMap(pInfo->idMap);			//? need not ?
...
bool CNpcManager::CreateSynPet(const ST_CREATENEWNPC* pInfo, LPCTSTR pszName/*=NULL*/)
{
	...
	CMonster* pPet = CMonster::CreateNew();
	if(pPet)
	{
		CNpcType*	pType = MonsterType()->GetObj(pInfo->usType);
		if(pType)
		{
			...
			CGameMap* pMap = MapManager()->QueryMap(pInfo->idMap);			//??? need not? mast check not current mapgroup case!!!
...
CMonster* CNpcManager::CreateCallPet(CUser* pUser, OBJID idType, int x, int y)
{
	...
	if(pPet)
	{
		...
		if(pType)
		{
			...
			CGameMap* pMap = pUser->GetMap();			//??? need not? mast check not current mapgroup case!!!
...
CMonster* CNpcManager::CreateEudemon(CUser* pUser, CItem* pItem, int x, int y)
{
	...
	CMonster* pEudemon = CMonster::CreateNew();
	if(pEudemon)
	{
