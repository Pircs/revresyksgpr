...
//	virtual bool	IsScene				()			{ return false; }
	...
	virtual void	BroadcastRoomMsg	(Msg* pMsg, bool bSendSelf);
	virtual void	BroadcastRoomMsg	(LPCTSTR szMsg, bool bSendSelf);
	virtual void	BroadcastMapMsg		(Msg* pMsg, bool bSendSelf);
...
//	static int	GetSex(DWORD dwLookFace)					{ return (m_dwLookFace/1000)%10; }
...
protected:
	virtual		uint64		SetEffect			(uint64 i64Effect)	{ m_data.m_Info.i64Effect |= i64Effect; return m_data.m_Info.i64Effect; }
	virtual		uint64		ResetEffect			(uint64 i64Effect=0xFFFFFFFFFFFFFFFF)	{ m_data.m_Info.i64Effect &= (~i64Effect); return m_data.m_Info.i64Effect; }
...
public: // item ------------------------------------------
	...
	CItem**	GetEquipItemPtr	(int nPosition);
	CItem*&	GetEquipItemRef	(int nPosition)						{ CItem** ppEquip = GetEquipItemPtr(nPosition); ASSERT(ppEquip); return *ppEquip; }
...
public: // friend --------------------------------
	...
	void		BroadcastFriendsMsg	(Msg* pMsg);
protected:
...
protected:
	...
	CItem*	m_pShoes;
	CItem*	m_pMount;
	CItem*	m_pSprite;	// 精灵 -- add by zlong 2003-11-27
	CItem*	m_pMantle;	// 披风 -- zlong 2004-02-04
