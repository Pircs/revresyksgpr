...
bool CMsgPackage::Create(int nAction,OBJID id , OBJID idItem, UCHAR ucType)
{
	...
	m_id			= id;
	m_ucAction	= nAction;
	m_ucType		= ucType;	//MSGPACKAGE_TYPE_NONE;						// 无意义
	m_idItem		= idItem;
...
/*				if(pUser->IsItemFull(pUser->QueryStorage()->GetWeight(idItem),
				...
				CItem* pItem = pUser->QueryStorage()->CheckOut(idNpc, idItem, UPDATE_FALSE,ITEMPOSITION_AUCTION_STORAGE);		// 先取，防作弊
				if(!pItem)
					return ;
		...
		case	MSGPACKAGE_CHECKOUT:
			{
				...
				// ITEM
				DEBUG_TRY	// VVVVVVVVVVVVVV
				CItem* pItem = pUser->QueryStorage()->CheckOut(idNpc, idItem, UPDATE_FALSE);		// 先取，防作弊
				if(!pItem)
					return ;
				pUser->AddItem(pItem, SYNCHRO_TRUE);
				DEBUG_CATCH("MSGPACKAGE_CHECKIN") // AAAAAAAAAAAAAAAAA
		...
		case	MSGPACKAGE_CHECKOUT:
			{
				...
				// ITEM
				DEBUG_TRY	// VVVVVVVVVVVVVV
				CItem* pItem = pNpc->QueryTrunk()->QueryPackage()->CheckOut(pUser, idItem, UPDATE_FALSE);		// 先取，防作弊
				if(!pItem)
					return ;
				pUser->AddItem(pItem, SYNCHRO_TRUE);
		...
		case	MSGPACKAGE_CHECKOUT:
			{
				...
				// ITEM
				DEBUG_TRY	// VVVVVVVVVVVVVV
				CItem* pItem = pUser->QueryStorage()->CheckOut(idChest, idItem, UPDATE_FALSE, ITEMPOSITION_CHEST);		// 先取，防作弊
				if(!pItem)
					return ;
				pUser->AddItem(pItem, SYNCHRO_TRUE);
				// log
			...
			case MSGPACKAGE_CHECKOUT:
				{
					...
					// ITEM
					DEBUG_TRY	// VVVVVVVVVVVVVV
					CItem* pItem = pPackage->CheckOut(pUser, idItem, UPDATE_FALSE);		// 先取，防作弊
					if(!pItem)
						return ;
