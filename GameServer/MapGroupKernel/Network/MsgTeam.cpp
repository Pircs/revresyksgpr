...
void CMsgTeam::Process(CUser* pUser)
{
	if (!pUser)
		return;
	...
	case _MSGTEAM_KICKOUT:
		{
			...
			if(pUser->GetID() != m_idPlayer)		// 不能开除自己
				pUser->KickOut(m_idPlayer);
		}
		break;
