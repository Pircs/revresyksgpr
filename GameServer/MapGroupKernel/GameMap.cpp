...
void CGameMap::BroadcastBlockMsg(IMapThing* pThing, Msg* pMsg, bool bSendSelf /*= false*/)
{
	ASSERT(pMsg);
...
void CGameMap::BroadcastBlockMsg(int nPosX, int nPosY, Msg* pMsg)
{
	ASSERT(pMsg);
...
void CGameMap::SendMapInfo(CUser* pUser)
{
	...
	MsgMapInfo msg;
	IF_OK (msg.Create(GetID(),GetDocID(),GetType()))
		pUser->SendMsg(&msg);
}
