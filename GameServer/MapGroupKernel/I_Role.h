...
const uint64		KEEPEFFECT_NORMAL			=0x00000000; 	// 无特殊状态

const uint64		KEEPEFFECT_TEAMLEADER		=0x00000001;    // 队长

const uint64		KEEPEFFECT_DIE				=0x00000002;	// 死亡
const uint64		KEEPEFFECT_GHOST			=0x00000004;    // 灵魂状态
const uint64		KEEPEFFECT_DISAPPEARING		=0x00000008;    // 尸体消失状态


const uint64		KEEPEFFECT_CRIME			=0x00000010;	// 犯罪 // 闪蓝色	
const uint64		KEEPEFFECT_RED				=0x00000020;	// 红名
const uint64		KEEPEFFECT_DEEPRED			=0x00000040;	// 黑名
const uint64		KEEPEFFECT_SYNCRIME			=0x00000080;	// 帮派犯罪

const uint64		KEEPEFFECT_POISON			=0x00000100;    // 中毒
const uint64		KEEPEFFECT_HIDDEN			=0x00000200;    // 隐身
const uint64		KEEPEFFECT_FREEZE			=0x00000400;    // 冰冻
const uint64		KEEPEFFECT_SUPERSOLDIER		=0x00000800;    // 无双
const uint64		KEEPEFFECT_LURKER			=0x00001000;    // 潜伏

const uint64		KEEPEFFECT_DEFENCE1			=0x00002000;    // 防御提高1
const uint64		KEEPEFFECT_DEFENCE2			=0x00004000;    // 防御提高2
const uint64		KEEPEFFECT_DEFENCE3			=0x00008000;    // 防御提高3

const uint64		KEEPEFFECT_ATTACK			=0x00010000;    // 攻击提升
const uint64		KEEPEFFECT_ATKSPEED			=0x00020000;    // 攻击速度提高

const uint64		KEEPEFFECT_MAGICDAMAGE		=0x00040000;    // 魔法伤害提高
const uint64		KEEPEFFECT_MAGICDEFENCE 	=0x00080000;    // 魔法防御提高

const uint64		KEEPEFFECT_REFLECT			=0x00100000;	// 攻击反射
const uint64		KEEPEFFECT_REFLECTMAGIC		=0x00200000;    // 魔法反射

const uint64		KEEPEFFECT_SLOWDOWN1		=0x00400000;    // 减速状态 50%
const uint64		KEEPEFFECT_SLOWDOWN2		=0x00800000;	// 减速状态 // 血少于一半时开始减速 // 50%

const uint64		KEEPEFFECT_TEAM_HEALTH		=0x01000000;	// 医疗结界状态
const uint64		KEEPEFFECT_TEAM_ATTACK		=0x02000000;	// 攻击结界状态
const uint64		KEEPEFFECT_TEAM_DEFENCE		=0x04000000;	// 护体结界状态
const uint64		KEEPEFFECT_TEAM_SPEED		=0x08000000;	// 速度结界状态
const uint64		KEEPEFFECT_TEAM_EXP			=0x10000000;	// 修炼结界状态
const uint64		KEEPEFFECT_TEAM_SPIRIT		=0x20000000;	// 心灵结界状态
const uint64		KEEPEFFECT_TEAM_CLEAN		=0x40000000;	// 净化结界状态

const uint64		KEEPEFFECT_SMOKE			=0x80000000;			// 烟雾效果
const uint64		KEEPEFFECT_DARKNESS			=0x0000000100000000;	// 黑暗效果
const uint64		KEEPEFFECT_PALSY			=0x0000000200000000;	// 麻痹效果
const uint64		KEEPEFFECT_MAXLIFE			=0x0000000400000000;	// 最大生命增加/减少
const uint64		KEEPEFFECT_MAXENERGY		=0x0000000800000000;	// 最大体力增加/减少

const uint64		KEEPEFFECT_ADD_EXP			=0x0000001000000000;	// 战斗经验增加
const uint64		KEEPEFFECT_ATTRACT_MONSTER	=0x0000002000000000;	// 吸引怪物
const uint64		KEEPEFFECT_XPFULL			=0x0000004000000000;	// XP满

const uint64		KEEPEFFECT_ADJUST_DODGE			=0x0000008000000000;	// 调节总的躲避值
const uint64		KEEPEFFECT_ADJUST_XP			=0x0000010000000000;	// 调节每次增加XP值
const uint64		KEEPEFFECT_ADJUST_DROPMONEY		=0x0000020000000000;	// 调节怪物每次掉钱

const uint64		KEEPEFFECT_PK_PROTECT			=0x0000040000000000;	// pk保护状态
const uint64		KEEPEFFECT_PELT					=0x0000080000000000;	// 疾行状态
const uint64		KEEPEFFECT_ADJUST_EXP			=0x0000100000000000;	// 战斗获得经验调整

const uint64		KEEPEFFECT_HEAL					=0x0000200000000000;	// 治愈状态
const uint64		KEEPEFFECT_FAINT				=0x0000400000000000;	// 晕
const uint64		KEEPEFFECT_TRUCULENCE			=0x0000800000000000;	// 野蛮
const uint64		KEEPEFFECT_DAMAGE				=0x0001000000000000;	// 调整受到的伤害
const uint64		KEEPEFFECT_ATKER_DAMAGE			=0x0002000000000000;	// 调整对目标造成的伤害
const uint64		KEEPEFFECT_CONFUSION			=0x0004000000000000;	// 混乱
const uint64		KEEPEFFECT_FRENZY				=0x0008000000000000;	// 狂暴
const uint64		KEEPEFFECT_EXTRA_POWER			=0x0010000000000000;	// 神力
const uint64		KEEPEFFECT_TRANSFER_SHIELD		=0x0020000000000000;	// 护盾
const uint64		KEEPEFFECT_SORB_REFLECT			=0x0040000000000000;	// 吸收反射
const uint64		KEEPEFFECT_FRENZY2				=0X0080000000000000;	// 另一种狂暴状态


// 综合状态
//enum {
const uint64		KEEPEFFECT_NOT_ACTIVE		=KEEPEFFECT_DIE|KEEPEFFECT_FREEZE|KEEPEFFECT_PALSY;
const uint64		KEEPEFFECT_NOT_CLEAR		=KEEPEFFECT_TEAMLEADER|KEEPEFFECT_RED|KEEPEFFECT_DEEPRED;		// 玩家死亡不清此状态
		// 结界系统所有状态
const uint64		KEEPEFFECT_TEAM_ALL			=KEEPEFFECT_TEAM_HEALTH|KEEPEFFECT_TEAM_ATTACK|KEEPEFFECT_TEAM_DEFENCE
...
//	virtual	BOOL	CheckStatus			(int nStatus)			PURE_VIRTUAL_FUNCTION_0
	virtual uint64		GetEffect			()						PURE_VIRTUAL_FUNCTION_0
...
public: // system
	virtual void	BroadcastRoomMsg(Msg* pMsg, bool bSendSelf = false)	PURE_VIRTUAL_FUNCTION
	virtual void	BroadcastRoomMsg(LPCTSTR szMsg, bool bSendSelf = false)	PURE_VIRTUAL_FUNCTION
//	virtual void	BroadcastMapMsg(Msg* pMsg, bool bSendSelf = false)	PURE_VIRTUAL_FUNCTION
