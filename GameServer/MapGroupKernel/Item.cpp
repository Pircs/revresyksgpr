...
UCHAR CItem::GetRelationShip(OBJID idDivine)
{
	...
	// 只要取其中指定的十进制位返回
	return (dwRelationShip / (DWORD)pow(10.0f, (float)(idDivine-1)) % 10);
}
...
bool CItem::AddRelationShip(OBJID idDivine, int nValue)
{
	...
	DWORD dwRelationShip = m_pEudemonData->GetInt(EUDEMONDATA_RELATIONSHIP);
	DWORD dwPow = (DWORD)pow(10.0f, (float)idDivine-1);
...
void CItem::WarGhostLevelUpgrade(CUser* pUser)
{
	if(!IsEquipment())
		return ;	
