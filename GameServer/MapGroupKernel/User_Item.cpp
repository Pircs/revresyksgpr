		...
		for(int i = 0; i < pRes->RecordCount(); i++)
		{
			...
			if(pItem)
			{
				if(pItem->Create(pRes, pDb))
				{
					...
					CItem*& refpEquip = GetEquipItemRef(nPosition);
...
{
	CItem** ppEquip = this->GetEquipItemPtr(nPosition);
	if (!ppEquip)
		return NULL;
...
CItem** CUser::GetEquipItemPtr(int nPosition)
{
	switch(nPosition)
	{
...
bool CUser::DelItem	(OBJID idItem, bool bSynchro)
{
	if (m_pPackage->DelItem(idItem))
	{
		...
		{
			MsgItem msg;
			if(msg.Create(idItem, ITEMACT_DROP, ITEMPOSITION_NONE))
				this->SendMsg(&msg);
		}
		return true;
	}
...
bool CUser::UnEquipItem(int nPosition, bool bSynchro)
{
	...
	CItem* pItem = UnEquipOnly(nPosition);
	CHECKF(pItem);
...
bool CUser::DropItem(OBJID idItem, int x, int y)
{
	...
	{
		CItem*& refpEquip = GetEquipItemRef(i);
...
{
	CHECKF(idItem);
	CItem* pItem = GetEquipItem(idItem);
	if(!pItem)
		pItem = GetItem(idItem);
...
bool CUser::Marry	(OBJID idTarget)
{
	...
	{
		MsgItem msg;
		IF_OK (msg.Create(this->GetID(), ITEMACT_FIREWORKS, ITEMPOSITION_NONE))
...
void CUser::AddEquipmentDurability(int nPosition, int nAddValue)
{
	...
	CItem*& refpEquip = this->GetEquipItemRef(nPosition);
	...
	{
		MsgItem msg;	// if nDurability equal zero, the client will auto delete this equipment
