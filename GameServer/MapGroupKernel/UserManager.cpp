...
#define  ACTION_LOGINPROMPT		1000000
...
bool CUserManager::LogoutUser(SOCKET_ID idSocket, BOOL bOffline/*=true*/)   
{
	...
	CUser* pUser = GetUserBySocketID(idSocket);
	if(pUser)
	{
...
CUser* CUserManager::GetUser(SOCKET_ID idSocket, OBJID idNpc)			// for agent
{
	//ASSERT(pMsg);
	...
	if(idSocket == SOCKET_NONE)
	{
		...
		return GetUserBySocketID(idSocket);
}
...
void CUserManager::BroadcastMsg(Msg* pMsg, CUser* pExcludeSender/*=NULL*/)
{
	CHECK(pMsg);
...
void CUserManager::BroadcastMapMsg(OBJID idMap, Msg* pMsg, CUser* pExcludeSender)
{
	ASSERT(pMsg);
...
void CUserManager::BroadcastMapGroupMsg(Msg* pMsg, CUser* pExcludeSender)
{
	ASSERT(pMsg);
