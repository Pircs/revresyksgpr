...
*/
			...
			if(m_tHammer < clock())
			{
				...
				if(m_nCountUp <= AUCTION_DEGREE_MAX)
				{
					...
					if(!m_isSysBid)
					{	
						CItem* pItem = QueryPackage()->GetItemByID(m_Current.idItem);
						CHECK(pItem);
...
*/			}
		...
		else//玩家物品
		{
			if(m_pProprietor->dwValue != m_Current.dwValue)//拍买成功
			{
				CItem* pItem = QueryPackage()->GetItemByID(m_pProprietor->idItem);
				if(!pItem)
				{
...
void CAuction::BroadcastListMsg(Msg* pMsg, CUser* pExcludeUser)  //广播消息
{
...
bool CAuction::ShoutPrice(OBJID idItem,OBJID idUser, int nMoney)
{
	...
	if(!m_isSysBid)
	{	
		CItem* pItem = QueryPackage()->GetItemByID(idItem);
		CHECKF(pItem);
