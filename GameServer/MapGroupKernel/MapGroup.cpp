...
bool CMapGroup::Create(PROCESS_ID idProcess, ISocket* pSocket, IDatabase* pDb, IMessagePort* pMsgPort)
{
	...
	m_pGameItemDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[0], false);
	m_pUserDataDefault			= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[1], false);
	m_pGameMapDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[2], false);
	m_pUserWeaponSkillDefault	= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[3], false);
	m_pMagicDataDefault			= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[4], false);
	m_pSynDataDefault			= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[5], false);
	m_pSynAttrDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[6], false);
	m_pPetDataDefault			= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[7], false);
	m_pDynaNpcDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[8], false);
	m_pDynaMapDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[9], false);
	m_pEudemonDataDefault		= ::MapGroup(PID)->GetDatabase()->CreateNewRecordset(szSQL[10], false);
...
#define  INTERVAL_EVENT	60	// seconds
...
bool CMapGroup::SendClientMsg(SOCKET_ID idSocket, Msg* pMsg)
{
	//pMsg->AppendInfo(m_idProcess, idSocket, ID_NONE);
	return m_pSocket->SendClientMsg(idSocket, pMsg);
}

bool CMapGroup::SendNpcMsg(OBJID idNpc, Msg* pMsg)
{
	//pMsg->AppendInfo(m_idProcess, SOCKET_NONE, idNpc);
	return m_pSocket->SendNpcMsg(idNpc, pMsg);
}

bool CMapGroup::SendBroadcastMsg(Msg* pMsg)
{
...
// 广播给其它地图组核心处理
bool CMapGroup::TransmitMsg(Msg* pMsg, SOCKET_ID idSocket, OBJID idNpc)
{
	ASSERT(idSocket != SOCKET_NONE || idNpc != ID_NONE);

	MESSAGESTR	buf;
	TRANSMSG_PACKET0* pPacket = (TRANSMSG_PACKET0*)buf;
	pPacket->Create(idSocket, idNpc, pMsg->GetTransData(), pMsg);
	for(int i = MSGPORT_MAPGROUP_FIRST; i < m_pMsgPort->GetSize(); i++)
	{
		if(i != m_idProcess)		// 不回送
...
// 广播给世界核心处理
bool CMapGroup::TransmitWorldMsg(Msg* pMsg)
{
	MESSAGESTR	buf;
	TRANSMSG_PACKET0* pPacket = (TRANSMSG_PACKET0*)buf;
	pPacket->Create( 0/*pMsg->GetSocketID()*/, 0/*pMsg->GetNpcID()*/,pMsg->GetTransData(),pMsg);
	m_pMsgPort->Send(MSGPORT_WORLD, WORLD_TRANSMITMSG, STRUCT_TYPE(buf), pPacket);
	return true;
}

// 通过世界核心转发给玩家
bool CMapGroup::TransmitWorldMsgByName(SOCKET_ID idSocket, Msg* pMsg, LPCTSTR szName)
{
	MESSAGESTR	buf;
	USERNAMEMSG_PACKET0* pPacket = (USERNAMEMSG_PACKET0*)buf;
	pPacket->Create(idSocket, szName, pMsg);
	m_pMsgPort->Send(MSGPORT_WORLD, WORLD_USERNAMEMSG, STRUCT_TYPE(buf), pPacket);
	return true;
}

// 通过世界核心转发给玩家
bool CMapGroup::TransmitWorldMsgByUserID(Msg* pMsg, OBJID idUser)
{
	MESSAGESTR	buf;
	USERIDMSG_PACKET0* pPacket = (USERIDMSG_PACKET0*)buf;
	pPacket->Create( 0/*pMsg->GetSocketID()*/, idUser, pMsg);
...
//bool CMapGroup::TransmitWorldMsg(SOCKET_ID idSocket,Msg *pMsg)
//{
//	MESSAGESTR	buf;
//	TRANSMSG_PACKET0* pPacket = (TRANSMSG_PACKET0*)buf;
//	pPacket->idSocket	= idSocket;
...
//	memcpy(pPacket->buf, pMsg, pMsg->GetSize());
