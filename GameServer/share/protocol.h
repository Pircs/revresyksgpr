...
{
	bool Create(SOCKET_ID idSocket, Msg* pMsg)
	{
		this->idSocket = idSocket;
		 memcpy(msgBuf, pMsg, pMsg->GetSize());
		 return true;
	}
	SOCKET_ID	idSocket;				// SOCKET_ID
	Msg			msgBuf[1];				// 用于结构指针，不需要空间
};

struct NPCMSG_PACKET0					// 接收的NPC消息
{
	bool Create(OBJID idNpc, Msg* pMsg)
	{
		this->idNpc = idNpc;
		memcpy(msgBuf, pMsg, pMsg->GetSize());
		return true;
...
{
	bool Create(SOCKET_ID idSocket, OBJID idNpc, int nTrans, Msg* pMsg)
	{
		this->idSocket = idSocket;
		this->idNpc = idNpc;
		this->nTrans = nTrans;
...
{
	bool Create(SOCKET_ID idSocket, LPCTSTR szName, Msg* pMsg)
	{
		this->idSocket = idSocket;
		SafeCopy(this->szName, szName, _MAX_NAMESIZE);
		memcpy(msgBuf, pMsg, pMsg->GetSize());
		return true;
	}
	SOCKET_ID	idSocket;				// SOCKET_ID
...
{
	bool Create(SOCKET_ID idSocket, OBJID idUser, Msg* pMsg)
	{
		this->idSocket = idSocket;
		this->idUser = idUser;
		memcpy(msgBuf, pMsg, pMsg->GetSize());
