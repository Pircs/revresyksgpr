	...
	case	SOCKET_SENDCLIENTMSG:
		{
			...
			if(pPack->idSocket >= m_setSocket.size())
				break;
			SendPacket(pPack->idSocket, pPack->msgBuf);
		}
		break;
	case	SOCKET_SENDNPCMSG:
		{
			...
			SendPacket(m_idNpcSocket, pPack->msgBuf);
		}
		break;
...
bool CSocketKernel::ProcessSocket()
{
	...
	for(int i = m_setSocket.size()-1; i >= 0; i--)
	{
		...
		if(pBuf&&nBufLen>sizeof(Msg)&&nBufLen<MAX_PACKETSIZE)
		{
...
#endif
					m_setSocket[i]->ClearPacket(pMsg->GetSize());
			...
			else // NPC服务器消息
			{
				do
				{
					NPCMSG_PACKET0* pPacket = (NPCMSG_PACKET0*)pBuf;
					if(pPacket->msgBuf->GetSize()>sizeof(Msg)&&pPacket->msgBuf->GetSize()<=nBufLen)
					{
						PROCESS_ID	idProcess = MSGPORT_WORLD;
						NPCPROCESSID_MAP::iterator iter = m_setNpcProcessID.find(pPacket->idNpc);
						if(iter != m_setNpcProcessID.end())
							idProcess = iter->second;
#ifdef	DUMP_DEBUG_MSG
						if(pPacket->msgBuf->GetType() == 1002)
							LOGDUMP("LOGIN AGENT %u in process id %d", pPacket->idNpc, idProcess);
#endif
						//char	bufPacket[MAX_MESSAGESIZE];
						//NPCMSG_PACKET0* pPacket = (NPCMSG_PACKET0*)bufPacket;
						//pPacket->Create(pPacket->idNpc, pMsg)
						m_pMsgPort->Send(idProcess, KERNEL_NPCMSG, STRUCT_TYPE(nBufLen), pBuf);

						int	nCurrMsgSize = PROTOCOL_NPCMSG_HEADSIZE+pPacket->msgBuf->GetSize();
#ifdef	DUMP_NPC_MSG
						char	szText[256];
						sprintf(szText, "AAA 接收了NPC消息%u", pPacket->msgBuf->GetType());
						m_pMsgPort->Send(MSGPORT_SHELL, SHELL_PRINTTEXT, STRING_TYPE(szText), szText);
#endif
...
bool CSocketKernel::SendPacket(SOCKET_ID idSocket, const Msg* pMsg)
{
	if(idSocket!=SOCKET_NONE&&m_setSocket[idSocket])
	{
#ifdef	PALED_DEBUG
		char	szText[256];
		sprintf(szText, "VVV 发送了消息%u", pMsg.GetType());
		m_pMsgPort->Send(MSGPORT_SHELL, SHELL_PRINTTEXT, STRING_TYPE(szText), szText);
#endif
		clock_t	clkDebug = clock();		// debug
		m_setSocket[idSocket]->SendPacket((const char*)pMsg, pMsg->GetSize());
		{
			extern struct STAT_STRUCT	g_stat;
			//						InterlockedExchangeAdd(&g_stat.setDebug[0], clock() - clkDebug);		// debug
			// stat
			InterlockedExchangeAdd(&g_stat.nSocketBytes, pMsg->GetSize());
