...
void CPlayer::OnTimer(time_t tCurr)
{
	...
	if(m_nLevel < LOGIN_FREE_LEVEL)
	{
		...
		if (m_tTick.ToNextTime(POINTFEE_SECS))
			GameWorld()->SendFee(m_idAccount, MsgFee::FEE_TICK);
		return;
	}
	...
	if(GameWorld()->IsAccountLoginOK() && time(NULL) > m_tPoint + POINTFEE_DELAY_SECS)
	{
		...
		GameWorld()->SendFee(m_idAccount, MsgFee::FEE_POINT);
	}
}

bool CPlayer::SendMsg(Msg* pMsg)
{
	if(IsAgent())
...
bool CUserList::LoginUser(LPCTSTR szAccount, LPCTSTR szPassword, SOCKET_ID idSocket)	// 仅用于非帐号服务器启动
{
	...
	if(pRes)
	{
		...
		GameWorld()->SendFee(idAccount, MsgFee::FEE_BEGIN);
	}
...
int CUserList::LoginUser(OBJID idAccount, SOCKET_ID idSocket, LPCTSTR szInfo)
{
	...
	if(pRes)
	{
		...
		{
			nRet = LOGIN_BAN;
			GameWorld()->SendFee(idAccount, MsgFee::FEE_OFFLINE);
		}
		pRes->Release();		//AAAAAAAAAAAAAAAAAAAAAAAAAA
...
bool CUserList::LogoutUser(SOCKET_ID idSocket)		// return false: no this user
{
	...
	CHECKF(pPlayer);
	GameWorld()->SendFee(pPlayer->m_idAccount, MsgFee::FEE_END);
	int	tCurr = time(NULL);
...
bool CUserList::KickOutAccount(OBJID idAccount, LPCTSTR szText)		// return false: 不在线
{
	...
	{
		CMsgTalk	msg;
		if(msg.Create(SYSTEM_NAME, ALLUSERS_NAME, (char*)szText, "", _COLOR_WHITE, _TXTATR_REGISTER))
			pPlayer->SendMsg(&msg);
	}
...
//	unLook = (unLook%10000)+10000;
	...
	SQLBUF	szSQL;
	sprintf(szSQL, "INSERT %s SET account_id=%u,name='%s',lookface=%d,hair=%d,length=%d,fat=%d,life=%d,mana=%d,money=%u,recordmap_id=%u, recordx=%u, recordy=%u,force1=%d,dexterity=%d,health=%d,soul=%d,profession=%d,additional_point=%d",
					_TBL_USER,
...
void CUserList::LoginToMapGroup(CPlayer* pPlayer)
{
	...
	{
		CMsgTalk	msg;
		if(msg.Create(SYSTEM_NAME, ALLUSERS_NAME, REPLAY_OK_STR, "", _COLOR_WHITE, _TXTATR_ENTRANCE))
			pPlayer->SendMsg(&msg);
	}
}

void CUserList::BroadcastMsg(Msg* pMsg, CPlayer* pExcludeSender/*=NULL*/)
{
	CHECK(pMsg);
