...
// 目前数据库中有空串帐号	ASSERT(pszLoginName[0]);
	...
	SafeCopy(m_szLoginName, pszLoginName, MAX_NAMESIZE);
	SafeCopy(m_szPassword, pszPassword, MAX_NAMESIZE);

	SafeCopy(m_szClientIP, pszClientIP, IPSTRSIZE);
	SafeCopy(m_szServerName, pszServerName, MAX_NAMESIZE);

	m_nFeeType			= nFeeType;
	m_idFeeAccount		= idFeeAccount;
	SafeCopy(m_szFeeAccount, szFeeAccount, MAX_NAMESIZE);
	m_tStartPoint		= 0;
	m_tLastPoint		= 0;
	m_tEndPoint			= 0;
					  ...
					  DWORD &nAuthenID, char * szServerName, char* szFeeAccount, OBJID &idFeeAccount, char* szOldIP)
{
	...
	if(nIndex && m_pAccount[nIndex].m_nState)
	{
		...
		SafeCopy(szPassword, m_pAccount[nIndex].m_szPassword, MAX_NAMESIZE);
		nAuthenID		= m_pAccount[nIndex].m_nAuthenID;
		SafeCopy(szServerName, m_pAccount[nIndex].m_szServerName, MAX_NAMESIZE);
		SafeCopy(szFeeAccount, m_pAccount[nIndex].m_szFeeAccount, MAX_NAMESIZE);
		idFeeAccount	= m_pAccount[nIndex].m_idFeeAccount;
		SafeCopy(szOldIP, m_pAccount[nIndex].m_szClientIP, MAX_NAMESIZE);
		return true;
	}
	return false;
}
...
{ 
	...
	if(nIndex)
	{
		SafeCopy(bufServer, m_pAccount[nIndex].m_szServerName, MAX_NAMESIZE);
		return true;
	}
	else
		return false;
