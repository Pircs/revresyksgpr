...
{
	try{
		if(!pszName || MAX_NAMESIZE <= strlen(pszName))
			return false;
...
void CAccount::SetAddress(const char* pszAddress, BOOL bUpdate)
{
	...
	{	
		...
		m_pRes->Fields("address") = pszAddress;
		m_pRes->Update();
	}
}
...
void CAccount::SetIdNumber(const char* pszIdNumber, BOOL bUpdate)
{
	...
	{	
		...
		m_pRes->Fields("idnumber") = pszIdNumber;
		m_pRes->Update();
	}
}
