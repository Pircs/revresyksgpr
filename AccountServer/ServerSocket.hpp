...
CServerSocket<key1, key2>::CServerSocket(/*uint32 key1, uint32 key2*/)	// 由于有监听SOCKET时初始化WSAStartup()，故该处不必调用
{
	m_sockServer	= INVALID_SOCKET;
	m_bufPeerIP[0]	= 0;
	m_tLastRecv		= 0;
...
//		SAVELOG("@@@: 已解密长度[%d][%d].", *(uint16 *)pBuf, nLen);
#endif
#ifdef	DUMP
				char buf[4096] = "";
				char buf3[4];
...
template <uint32 key1, uint32 key2>
bool CServerSocket<key1, key2>::Send(const Msg* pMsg)	// 返回false: 网络错误
{
...
{
//		SAVELOG("@@@: 已解密长度Send[%d][%d].", *(uint16 *)pBuf, nLen);
	...
	if(!(nMsgLen > 0 && nMsgLen <= _MAX_MSGSIZE && nMsgLen == nLen))		// 检查通讯消息完整性
	{
		...
		LOGERROR("该消息ID为[%d]", *(const uint16 *)(pBuf + 2));
		Close(true);
		return false;
	}
