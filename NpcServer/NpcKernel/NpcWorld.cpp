...
void CNpcWorld::ProcessMsg(OBJID idPacket, const char* pbufMsg, int nSize, OBJID idNpc)
{
	...
	{
		try {
			Msg* pMsg = (Msg*)pbufMsg;
			switch(pMsg->GetType())
			{
			case _MSG_PLAYER:
				((CMsgPlayer*)pMsg)->Process(idNpc);
				break;
			case _MSG_TALK:
				((CMsgTalk*)pMsg)->Process(idNpc);
				break;
			case _MSG_AINPCINFO:
				((CMsgAiNpcInfo*)pMsg)->Process();
				break;
			case _MSG_ACTION:
				((CMsgAction*)pMsg)->Process(idNpc);
				break;
			case _MSG_INTERACT:
				((CMsgInteract*)pMsg)->Process();
				break;
			case _MSG_WALK:
				((CMsgWalk*)pMsg)->Process();
				break;
			case _MSG_USERATTRIB:
				((CMsgUserAttrib*)pMsg)->Process();
				break;
			case _MSG_SYNDICATE:
				((CMsgSyndicate*)pMsg)->Process(idNpc);
				break;
			case _MSG_MAGICEFFECT:
				((CMsgMagicEffect*)pMsg)->Process();
				break;
			case _MSG_USERINFO:
				((CMsgUserInfo*)pMsg)->Process();
				break;
			case _MSG_ITEM:
				((CMsgItem*)pMsg)->Process(idNpc);
				break;
			case _MSG_ITEMINFO:
				((CMsgItemInfo*)pMsg)->Process(idNpc);
				break;
			case _MSG_TEAM:
				((CMsgTeam*)pMsg)->Process(idNpc);
				break;
			case _MSG_TEAMMEMBER:
				((CMsgTeamMember*)pMsg)->Process(idNpc);
				break;
			case _MSG_WALKEX:
				((CMsgWalkEx*)pMsg)->Process();
				break;
			default:
				break;
			}
...
void CNpcWorld::OnTimer(DWORD nCurr)
{
	...
	if(m_nLoginState == STATE_NORMAL)
	{
		...
		if(m_tUpdateStateWin.ToNextTime())
		{
			...
			{
				CNpcGenerator* pGen = NpcManager()->QueryGenSet()->GetObjByIndex(i);
				if(pGen)
				{
	...
	else if(m_nLoginState == STATE_NONE)
	{
		...
		MsgLogin	msg;
		if(msg.Create(NPCSERVER_ACCOUNT, NPCSERVER_PASSWORD,0))
...
bool CNpcWorld::SendMsg(OBJID idNpc, Msg* pMsg)
{
#ifdef	DUMP_DEBUG_MSG
...
#endif
	DEBUG_TRY
		if(!m_pShell->SendPacket((const char*)pMsg, pMsg->GetSize()))
